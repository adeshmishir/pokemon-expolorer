...
export default function SearchBar({ value, onChange, onSubmit, onClear, suggestions = [] }) {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full max-w-lg mx-auto mb-8">
      <div role="search" aria-label="Search Pokémon">
        <div className="flex items-center gap-2 rounded-lg border border-[var(--color-pokedex-border)] bg-[var(--color-pokedex-panel)] px-3 py-2.5 transition-all focus-within:border-[var(--color-pokeball-red)]/40 focus-within:shadow-[0_0_0_3px_rgba(220,38,38,0.08)] dark:border-[var(--color-pokedex-dark-border)] dark:bg-[var(--color-pokedex-dark-panel)] dark:focus-within:border-[var(--color-pokeball-red)]/30 dark:focus-within:shadow-[0_0_0_3px_rgba(220,38,38,0.12)]">
          ...
          {value && (
            <button
              type="button"
              onClick={() => { onClear(); setShowSuggestions(false); inputRef.current?.focus(); }}
