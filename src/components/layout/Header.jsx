...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--color-pokedex-border)] bg-[var(--color-pokedex-panel)]/80 backdrop-blur-md nav-glow dark:border-[var(--color-pokedex-dark-border)] dark:bg-[var(--color-pokedex-dark-surface)]/80">
      <div className="mx-auto flex h-14 w-full max-w-6xl items-center justify-between gap-4 px-4">
        ...
        >
          {/* Poké Ball icon */}
          <span className="relative flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-[var(--color-pokedex-text)] transition-transform duration-300 group-hover:scale-110 dark:border-[var(--color-pokedex-dark-text)]">
          ...
          {NAV_ITEMS.map(({ to, label, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                to={to}
                className={cn(
                  "relative flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold uppercase tracking-wider transition-all duration-200",
                  isActive
                    ? "nav-active-indicator bg-[var(--color-pokeball-red)]/10 text-[var(--color-pokeball-red)]"
      ...
      {/* Mobile bottom nav */}
      <nav
        className="fixed bottom-0 inset-x-0 z-50 flex border-t border-[var(--color-pokedex-border)] bg-[var(--color-pokedex-panel)]/95 backdrop-blur-md nav-glow sm:hidden dark:border-[var(--color-pokedex-dark-border)] dark:bg-[var(--color-pokedex-dark-surface)]/95"
        ...
        {NAV_ITEMS.map(({ to, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              to={to}
              className={cn(
                "flex flex-1 flex-col items-center gap-0.5 py-2.5 text-[10px] font-bold uppercase tracking-wider transition-colors duration-200",
                isActive
